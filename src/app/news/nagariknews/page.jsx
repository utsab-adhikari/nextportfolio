...
const EkantipurPage = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        const response = await axios.get("/api/v1/news/nagarik");
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-4 flex justify-center items-center flex-col text-center">
        <h1 className="text-2xl font-bold text-indigo-800 dark:text-indigo-400">Nagarik News</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          {Array.isArray(newses) && newses.length > 0 ? (
            <NewsGrid posts={newses} buttonLabel="Read at Nagarik News" />
          ) : (
